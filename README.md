...
Количество одновременных загруженных полноразмерных изображений управляется
файлом `cache_config.toml` в корне проекта. Параметр `max_loaded_images`
определяет максимальное число объектов в памяти. Значение `0` отключает
ограничение.
